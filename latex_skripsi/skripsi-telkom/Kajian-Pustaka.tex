...
Di samping optimasi struktural, penyempurnaan pada fungsi kerugian (\textit{loss function}) juga mutlak diperlukan pada tahap pelatihan. Mengingat distribusi data kerusakan jalan (seperti lubang jalan) sering kali mengalami ketimpangan ekstrem (\textit{class imbalance}), fungsi kerugian adaptif seperti \textit{Focal Loss} atau \textit{Refined Generalized Focal Loss} (REG) diterapkan untuk memaksa model memberikan pembobotan penalti yang lebih besar pada kelas minoritas yang sulit dideteksi \cite{panboonyuen2025reg}. Penggabungan teknik atensi, konvolusi ringan, dan fungsi kerugian adaptif ini merupakan landasan utama dalam menghasilkan model deteksi yang komprehensif, efisien, dan siap diimplementasikan secara nyata.
